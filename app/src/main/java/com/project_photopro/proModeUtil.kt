...
fun changeProModeValue(preferences: SharedPreferences) {
    ...
    preferences.edit {
        putInt(SharedPrefs.PRO_MODE_KEY, newValue)
    }
}

//The sliders are considered available if commands are shown
...
fun handleISOSlider(activity: MainActivity, preferences: SharedPreferences) {
    ...
    isoTextView.text = String.format(Locale.getDefault(), "%d", Constant.ISO_VALUES[savedISOValueIndex])
    ...
    isoSlider.addOnChangeListener { _, valueIndex, _ ->
        //Retrieve shown value from list
        isoTextView.text = String.format(Locale.getDefault(), "%d", Constant.ISO_VALUES[valueIndex.toInt()])

        //Save new value (index)
        preferences.edit {
            if (preferences.getInt(SharedPrefs.CAMERA_FACING_KEY, Constant.CAMERA_BACK) == Constant.CAMERA_BACK) {
                putInt(SharedPrefs.ISO_INDEX_BACK_KEY, valueIndex.toInt())
            } else {
                putInt(SharedPrefs.ISO_INDEX_FRONT_KEY, valueIndex.toInt())
            }
        }
...
fun handleShutterSpeedSlider(activity: MainActivity, preferences: SharedPreferences) {
    ...
    val savedShutterSpeedValueIndex =
        if (preferences.getInt(SharedPrefs.CAMERA_FACING_KEY, Constant.CAMERA_BACK) == Constant.CAMERA_BACK) {
            ...
            preferences.getInt(
                SharedPrefs.SHUTTER_SPEED_INDEX_FRONT_KEY,
                proRanges.frontExposureTimeRangeInList!!.lower
            )
        }

    //Retrieve shown value from list
    shutterSpeedTextView.text =
        convertNanosecondsToReadableTime(Constant.SHUTTER_SPEED_VALUES[savedShutterSpeedValueIndex])
    ...
    shutterSpeedSlider.addOnChangeListener { _, valueIndex, _ ->
        //Retrieve shown value from list
        shutterSpeedTextView.text = convertNanosecondsToReadableTime(Constant.SHUTTER_SPEED_VALUES[valueIndex.toInt()])

        //Save new value (index)
        preferences.edit {
            if (preferences.getInt(SharedPrefs.CAMERA_FACING_KEY, Constant.CAMERA_BACK) == Constant.CAMERA_BACK) {
                putInt(SharedPrefs.SHUTTER_SPEED_INDEX_BACK_KEY, valueIndex.toInt())
            } else {
                putInt(SharedPrefs.SHUTTER_SPEED_INDEX_FRONT_KEY, valueIndex.toInt())
            }
        }
    ...
    Thread {
        ...
        if (preferences.getInt(SharedPrefs.CAMERA_FACING_KEY, Constant.CAMERA_BACK) == Constant.CAMERA_BACK) {
            ...
            savedShutterSpeedValueIndex =
                preferences.getInt(
                    SharedPrefs.SHUTTER_SPEED_INDEX_BACK_KEY,
                    proRanges.backExposureTimeRangeInList!!.lower
                )
        ...
        val newCaptureRequestOptions = CaptureRequestOptions.Builder()
            ...
            .setCaptureRequestOption(
                ...
                Constant.SHUTTER_SPEED_VALUES[savedShutterSpeedValueIndex].toLong()
            )
            .build()
...
fun getProModeSliderRanges(activity: AppCompatActivity): ProModeRanges {
    ...
    ranges.frontExposureTimeRangeInList =
        getRangeInList_FL(Constant.SHUTTER_SPEED_VALUES, ranges.frontExposureTimeRange)
    ...
    ranges.backExposureTimeRangeInList = getRangeInList_FL(Constant.SHUTTER_SPEED_VALUES, ranges.backExposureTimeRange)
...
fun getRangeInList_II(list: List<Int>, range: Range<Int>?): Range<Int>? {
    ...
    //Check if the list has at least one value that is compatible with the range given
    var rangeEmpty = true
    for (value in list) {
        if (value in range) {
            rangeEmpty = false
